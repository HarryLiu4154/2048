...
class _GameState extends State<Game> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    ...
    stackItems.addAll([_flatGrid, _toAdd]
    ...
    bool canSwipeDown() =>
        _columns.map((e) => e.reversed.toList()).any(canSwipe);
    ...
    void addNewTile() {
      List<Tile> empty =
          _flatGrid.where((element) => element.value == 0).toList();
      empty.shuffle();
      ...
      _toAdd.add(
          Tile(empty.first.x, empty.first.y, newTileNum)..appear(_controller));
    }
